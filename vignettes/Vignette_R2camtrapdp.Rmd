...
## Create datapackages
Create an R6 object with named "datapackage"
```{r}
datapackage<-R6_CamtrapDP$new()
```

#Import tibble data (deployment, media and observation) into "datapackage"
...
#Import metadata (e.g., contributors, licences, and properties) into "datapackage"
For the metadata of Camera Trap Data Package (Camtrap DP), five properties (contributors, project, spatial, temporal and taxonomic, and created) are required. In addition, fifteen properties can be used for additional information of metadata.
...
1.Licenses
The information of licenses related to "datapackage" can be listed as metadata. If provide, "Camtrap DP" further requires at least a license for content of the "package" and one for the medis files. The function "add_license()" can be used as below,
```{r}
...
The function "add_relatedIdentifiers()" can be used to import related identifiers. The related identifiers need to set relatonType, relatedIdentifier, resourceTypeGeneral, and relatedIdentifierType.
relationType: Description of the relationship between the resource (the "package") and the related resource. Available from IsCitedBy, Cites, IsSupplementTo, IsSupplementedBy, IsContinuedBy, Continues, IsNewVersionOf, IsPreviousVersionOf, IsPartOf, HasPart, IsPublishedIn, IsReferencedBy, References, IsDocumentedBy, Documents, IsCompiledBy, Compiles, IsVariantFormOf, IsOriginalFormOf, IsIdenticalTo, HasMetadata, IsMetadataFor, Reviews, IsReviewedBy, IsDerivedFrom, IsSourceOf, Describes, IsDescribedBy, HasVersion, IsVersionOf, Requires, IsRequiredBy, Obsoletes, and IsObsoletedBy.
...
datapackage$add_relatedIdentifiers(relationType = c("IsSupplementTo"),
                                   relatedIdentifier =as.character(c("xxxxxx")) ,
