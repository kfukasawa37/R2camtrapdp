...
##settings
#media ID
mediaID<-paste(Iobs$institutionCode,
               Iobs$collectionCode,
               Iobs$locationID,
               as.numeric(factor(Vobs$filename)),
               sep="_")
#add deployment ID
Iobs<-Iobs%>%left_join(Idep%>%select(deploymentID,locationID),by=c("locationID"))
...
observationID<-paste(mediaID,Vobs$VideoID,sep="_")
...
#create observationIDs that express the unique identifier of the observation
observationID<-paste(mediaID,Iobs$obsID,sep="_")
#deploymentID is belong to deployment
deploymentID<-Vdep$deploymentID
#eventID is belong to the event
eventID<-Iobs$eventID

#Timestamp (date and time) at which the observation started (e.g., the timestamp when the video or photo was taken) and event start time and event end time (evnt-based observation)
tz="Japan"
timeDF<-Iobs%>%
  mutate(timestamp=paste(Iobs$date,Iobs$time))%>%
  group_by(eventID)%>%
  summarise(eventStart=min(timestamp),
         eventEnd=max(timestamp))
Iobs<-Iobs%>%left_join(timeDF,by=c("eventID"))
